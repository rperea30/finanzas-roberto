-- Deja todo limpio: borra TODOS los gastos, importa solo tus 14 compras reales
-- de Visa Global Bank, pone Yappy/Efectivo en $0, y Salario en $379.34.
...
begin
  ...
  -- 1) Borra TODOS los gastos registrados
  delete from expenses where household_id = v_household_id;

  -- 2) Yappy/Efectivo en $0
  update payment_methods set starting_balance = 0
  where household_id = v_household_id and name = 'Yappy / Efectivo';

  -- 3) Salario en 379.34 (meta y el ingreso ya recibido)
  select id into v_salario_id from income_sources where household_id = v_household_id and name = 'Salario';
  update income_sources set monthly_amount = 379.34 where id = v_salario_id;
  update income_entries set amount = 379.34
  where household_id = v_household_id and income_source_id = v_salario_id
    and notes = 'Efectivo (Yappy) inicial migrado del Excel';

  -- 4) Importa tus 14 compras reales de Visa Global Bank
